...
const CreateUpdateCampaign = (props) => {
    ...
    const handleSelectProduct = (selectPayload) => {
        ...
        var listSelectedProduct = selectPayload.selection.map((pro) => {
            return pro.id.replace('gid://shopify/Product/', '');
        });
        var listProductDetail = campaign.ListDetails.map((pro) => {
            return pro.ProductID;
        });
        var arrPro = campaign.ListDetails.filter(p => listSelectedProduct.indexOf(p.ProductID.toString()) >= 0);
        var arrAddedPro = selectPayload.selection.filter(p => listProductDetail.indexOf(p.id.replace('gid://shopify/Product/', '')) < 0).map((pro) => {
            return {
                ProductID: pro.id.replace('gid://shopify/Product/', ''),
                ProductTitle: pro.title,
                ProductPrice: pro.variants[0].price,
                ProductHandle: pro.handle,
                ProductImage: pro.images.length > 0 ? pro.images[0].originalSrc : '',
                Percentage: '0',
                Inventory: '0'
            };
        });
                    ...
                    <div className='campaign-products'>
                        {
                            (campaign.ListDetails == null || campaign.ListDetails.length == 0) ?
                                <div className='campaign-products-blank'>
                                    <Button primary onClick={() => {setIsOpenProductPicker(true)}}>+ Add products to your campaign</Button>
                                ...
                                <div className='campaign-products-list'>
                                    <div className='campaign-product-list-control'>
                                        ...
                                        <div className='campaign-product-list-control-button'>
                                            <Button primary  onClick={() => {setIsOpenProductPicker(true)}}>+ Add more product</Button>
                                        </div>
                        ...
                        <Provider config={configResourcePicker}>
                            <ResourcePicker resourceType="Product" open={IsOpenProductPicker}
